...
```{r doinfpan,eval=FALSE}
if (interactive() & nchar(Sys.getenv("CGC_BILLING")) > 0) {
...
bq = pancan_BQ()
#stad = bindMSI(buildPancanSE(bq, "STAD", assay="RNASeqv2"))
#stad = replaceRownames(stad)
...
library(ggplot2)
png(file="microsatpan.png")
ggplot(subset(rr, msicode %in% c("msi-h", "msi-l")), 
   aes(msicode, log2ex)) + geom_boxplot() + 
   facet_grid(acronym~alias) + 
   ylab("log2(normalized expr. + 1)") + 
   xlab("microsatellite instability code")
dev.off()
}
```
