...
# pcbq %>% dbListTables() # list all tables
clin = pcbq %>% tbl("clinical_PANCAN_patient_with_followup")
clinBRCA = clin %>% filter(acronym=="BRCA")
adf = clinBRCA %>% as.data.frame(n=1099)
clinBRCA_DF = DataFrame(adf)
rownames(clinBRCA_DF) = clinBRCA_DF[,2]  # sample names
library(SummarizedExperiment)
m = BQ3_Source( pcbq, "pancanMiRs_EBadjOnProtocolPlatformWithoutRepsWithUnCorrectMiRs_08_04_16_annot", 
   "ID", "ParticipantBarcode", "Study", "BRCA", "miRNAexpr")  
mq = BQ3_Array(m)
BRCA_mir = SummarizedExperiment(assays=SimpleList(mirna=mq), colData=clinBRCA_DF[m@allcolnames,])
BRCA_mir
```
The result is
...
metadata(0):
assays(1): mirna
rownames(743): hsa-miR-191-3p hsa-miR-3065-3p ... hsa-miR-4445-5p
  hsa-miR-515-3p
rowData names(0):
colnames(975): TCGA-3C-AALI TCGA-5T-A9QA ... TCGA-UL-AAZ6 TCGA-XX-A89A
